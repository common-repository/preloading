<?php
tbpl_import_class( 'Mobile_Detect', 'inc/mobile-detect/Mobile_Detect.php' );
...
class Pre_Loading {
	...
	public function __construct() {
		...
		if ( isset( $_GET['mode'] ) && $_GET['mode'] == 'tbpl_preview' ) {
			
			$this->settings	= get_option( 'pre_loading_options_preview' );
			$this->state	= ( ! current_user_can( 'manage_options' ) || ! file_exists( PRE_LOADING_ROOT_PATH . 'templates/' . $this->settings['template'] . '/pre-loading.php' ) ) ? false : 'preview';
			
		} else {
			
			$this->settings				= get_option( 'pre_loading_options' );
			$config_options				= get_option( 'pre_loading_config' );
	...
	public function splash_page() {
		...
		$this->template_url	= esc_url( PRE_LOADING_ROOT_URL . 'templates/' . $this->settings['template'] . '/' );
		...
		require_once( PRE_LOADING_ROOT_PATH . 'templates/' . $this->settings['template'] . '/pre-loading.php' );
		exit();
	...
	private function test() {
		...
		if ( ! $this->settings['enable_pre_loading_page'] )
			return false;
		...
		if ( ! file_exists( PRE_LOADING_ROOT_PATH . 'templates/' . $this->settings['template'] . '/pre-loading.php' ) )
			return false;		
	...
	private function form_validation() {
		...
		if ( $this->settings['enable_age_confirmation']  && isset( $_POST['tbpl-nonce'] ) && wp_verify_nonce( $_POST['tbpl-nonce'], PRE_LOADING_FORM_NONCE ) ) {		
			
			if (  isset( $_POST['tbpl-year'] ) && ctype_digit( $_POST['tbpl-year'] ) && isset( $_POST['tbpl-month'] ) && ctype_digit( $_POST['tbpl-month'] ) && isset( $_POST['tbpl-day'] ) && ctype_digit( $_POST['tbpl-day'] ) && $this->check_age( $_POST['tbpl-day'], $_POST['tbpl-month'], $_POST['tbpl-year'] ) ) {
		...
		if ( $this->settings['enable_opt_in'] && isset( $_POST['tbpl-nonce'] ) && wp_verify_nonce( $_POST['tbpl-nonce'], PRE_LOADING_FORM_NONCE ) ) {
	...
	private function check_ip(){
		...
		// Check Ip's
		$result	= $wpdb->get_var( $wpdb->prepare( 'SELECT ip FROM ' . PRE_LOADING_TABLE_IPS . ' WHERE ip = %s AND cookie_name = %s', array( $this->get_ip(), $this->cookie_name ) ) );
	...
	private function delete_ip( $ip = false ){
		...
		if( ! empty( $ip ) ) {
			$query	= 'DELETE FROM ' . PRE_LOADING_TABLE_IPS . ' WHERE ip = %s';
			$args	= $ip;
		
		}else{
			$query	= 'DELETE FROM ' . PRE_LOADING_TABLE_IPS . ' WHERE splash_date < %s';
