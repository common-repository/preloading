jQuery( function() {
	...
	jQuery( '#pre-loading-preview' ).click( function( event ) {	
		...
		data += '&nonce=' + pre_loading_data.previewSecurity;
		
		jQuery.ajax({
		
			url:		pre_loading_data.ajaxurl,
			data:		data + '&action=pre_loading_hook',
			type:		'POST',
			async:		false,
			dataType:	'json',
			...
			success: function( result ) {
				...
				window.open( result.homeURL + '?mode=tbpl_preview', '_newtab' );
